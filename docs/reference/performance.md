...
Results (Linux, GitHub Actions `ubuntu-latest`, CPython 3.13, 3 workers, best of
5), requests/sec. The table and the chart above are regenerated by the
...
RPC calls (1 to 10 KiB), **zloop matches or beats uvloop**, with the most
reproducible margin on the `buffered` path.
